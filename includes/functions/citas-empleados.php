...
if (isset($_POST['nuevaFecha'])) {
    ...
    // Obtén todas las citas para la fecha y el barbero específicos
    $queryCitasPendientes = mysqli_query($conexion, "SELECT * FROM cita WHERE idBarbero='$idBarbero' AND fecha='$nuevaFecha' AND estado='Pendiente' ORDER BY hora ASC");
    $citasPendientes = mysqli_fetch_all($queryCitasPendientes, MYSQLI_ASSOC);
    $queryCitasRealizadas = mysqli_query($conexion, "SELECT * FROM cita WHERE idBarbero='$idBarbero' AND fecha='$nuevaFecha' AND estado='Realizada' ORDER BY hora ASC");
    $citasRealizadas = mysqli_fetch_all($queryCitasRealizadas, MYSQLI_ASSOC);
    $queryCitasRealizadas = mysqli_query($conexion, "SELECT * FROM cita WHERE idBarbero='$idBarbero' AND fecha='$nuevaFecha' AND estado='Cancelada' ORDER BY hora ASC");
    $citasCanceladas = mysqli_fetch_all($queryCitasRealizadas, MYSQLI_ASSOC);

    $todasLasCitas = array_merge($citasPendientes, $citasRealizadas, $citasCanceladas);
    ...
    if (empty($todasLasCitas)) {
        ...
        foreach ($todasLasCitas as $cita) {
            ...
            $queryBarbero = mysqli_query($conexion, "SELECT comision FROM barberos WHERE id='$idBarbero'");
            ...
            $fechaHoy = date('Y-m-d');
            $queryTipo = mysqli_query($conexion, "SELECT tipo FROM accesobarberos WHERE id='$idBarbero'");
            $tipoBarbero = mysqli_fetch_assoc($queryTipo)['tipo'];
           if ($fechaHoy == $cita['fecha'] && $cita['estado']=="pendiente") {
    ...
    echo 'Error: Falta el parámetro de fecha';
}
